"""Native cognis-connect emit for k8saudit — forward findings to any platform.

Maps k8saudit's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    k8saudit ... --format json | k8saudit-emit --to stix
    k8saudit-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out["title"] = rec.get("rule", "unknown")

...
**k8saudit** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `k8saudit` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
k8saudit ... --format json | cognis-connect emit --to sigma   # Sigma rules
k8saudit ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
k8saudit ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
k8saudit ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `k8saudit` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="k8saudit") for rec in records]   # records = your JSON output

# Comprehensive installer for cognis-digital/k8saudit (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# k8saudit is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "k8saudit"
$Url  = "git+https://github.com/cognis-digital/k8saudit.git"
$Git  = "https://github.com/cognis-digital/k8saudit.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: k8saudit"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: k8saudit"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: k8saudit"; exit 0 }
}
